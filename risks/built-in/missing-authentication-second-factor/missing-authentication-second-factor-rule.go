			...
			for _, commLink := range commLinks {
				...
				if caller.UsedAsClientByHuman {
					...
					if moreRisky && commLink.Authentication != types.TwoFactor {
						risks = append(risks, missing_authentication.CreateRisk(technicalAsset, commLink, commLink, "", types.MediumImpact, types.Unlikely, true, Category()))
					}
					...
					for _, callersCommLink := range callersCommLinks {
						...
						if callersCaller.UsedAsClientByHuman {
							...
							if moreRisky && callersCommLink.Authentication != types.TwoFactor {
								risks = append(risks, missing_authentication.CreateRisk(technicalAsset, commLink, callersCommLink, caller.Title, types.MediumImpact, types.Unlikely, true, Category()))
							}
						}
					}
				}
